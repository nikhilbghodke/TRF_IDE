...
Blockly.cake.controls_for = function(a) {
    ...
    var c = Blockly.cake.valueToCode(a, "FROM", Blockly.cake.ORDER_ASSIGNMENT) || "0",
        ...
        g = Blockly.cake.statementToCode(a, "DO"),
        g = Blockly.cake.addLoopTrap(g, a.id);
    a = "for (" + b + " = " + c +
        "; " + b + "<" + d + "; " + b;
    b = 0 <= e;
    e = Math.abs(parseFloat(e));
    return a = (1 == e ? a + (b ? "++" : "--") : a + ((b ? " += " : " -= ") + e)) + (") {\n" + g + "}\n")
};
...
Blockly.cake.main_block = function(a) {
    ...
    for (var c = Blockly.cake.valueToCode(a, "RETURN", Blockly.cake.ORDER_NONE) || "", c = c ? "  return " + c + ";\n" : "  return 0;\n", d = [], e = [], g = [], h = 0; h < a.arguments_.length; h++) d[h] = Blockly.cake.variableDB_.getName(a.arguments_[h],
        Blockly.Variables.NAME_TYPE), e[h] = a.types_[h], g[h] = e[h] + " " + d[h];
    var d = [],
        e = [],
        f;
    for (f in Blockly.cake.times_) h = Blockly.cake.times_[f], f.match("srand") ? "main_block" == h[0] && (h = Blockly.cake.prefixLines(h[1], Blockly.cake.INDENT), d.push(h)) : f.match("time") && "main_block" == h[0] && (h = Blockly.cake.prefixLines(h[1], Blockly.cake.INDENT), e.push(h));
    f = d.length ? d.join("\n") + "\n" + e.join("\n") : e.join("\n");
...
Blockly.cake.procedures_defreturn = function(a) {
    ...
    var d = Blockly.cake.valueToCode(a, "RETURN", Blockly.cake.ORDER_NONE) || "",
        d = d ? "  return " +
        d + ";\n" : "  return 0;\n",
        e = Blockly.Procedures.getTypePlusArgs(a),
        g = [],
        h = [],
        f;
    for (f in Blockly.cake.times_) {
        var k = Blockly.cake.times_[f];
        f.match("srand") ? k[0] == b && (k = Blockly.cake.prefixLines(k[1], Blockly.cake.INDENT), g.push(k)) : f.match("time") && k[0] == b && (k = Blockly.cake.prefixLines(k[1], Blockly.cake.INDENT), h.push(k))
    }
    h = g.length ? g.join("\n") + "\n" + h.join("\n") : h.join("\n");
    g = a.getFieldValue("TYPES");
    f = a.getFieldValue("DISTS");
    "pointer" == f ? (f = a.getFieldValue("PSPECS"), null == f && (f = "*"), c = g + f + " " + b + "(" +
        e.join(", ") + ") {\n" + h.replace(/\n\n+/g, "\n\n").replace(/\n*$/, "\n") + c + d + "}") : "array" == f ? (f = a.getFieldValue("ASPECS"), null == f && (f = "[]"), c = g + f + " " + b + "(" + e.join(", ") + ") {\n" + h.replace(/\n\n+/g, "\n\n").replace(/\n*$/, "\n") + c + d + "}") : c = g + " " + b + "(" + e.join(", ") + ") {\n" + h.replace(/\n\n+/g, "\n\n").replace(/\n*$/, "\n") + c + d + "}";
    c = Blockly.cake.scrub_(a, c);
    Blockly.cake.definitions_[b] = c;
    Blockly.cake.definitions_["Func_declare" + b] = g + " " + b + "(" + e.join(", ") + ");"; - 1 == Blockly.Blocks.checkLegalName(Blockly.Msg.PROCEDURES_ILLEGALNAME,
        b) && this.initName();
    return null
};
Blockly.cake.procedures_defnoreturn = function(a) {
    ...
    for (g in Blockly.cake.times_) {
        var h = Blockly.cake.times_[g];
        g.match("srand") ? h[0] == b && (h = Blockly.cake.prefixLines(h[1], Blockly.cake.INDENT), d.push(h)) : g.match("time") && h[0] == b && (h = Blockly.cake.prefixLines(h[1], Blockly.cake.INDENT), e.push(h))
    }
    e = d.length ? d.join("\n") + "\n" + e.join("\n") : e.join("\n");
    ...
    Blockly.cake.INFINITE_LOOP_TRAP && (c = Blockly.cake.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + a.id + "'") + c);
    (g = Blockly.cake.valueToCode(a, "RETURN", Blockly.cake.ORDER_NONE) || "") && (g = "  return " + g + ";\n");
...
Blockly.cake.library_stdio_printf = function(a) {
    for (var b = "", c = "", d = "", e = "", g = 0; g <= a.printAddCount_; g++) {
        var b = Blockly.cake.valueToCode(a, "VAR" + g, Blockly.cake.ORDER_NONE) || "",
            c = a.inputList[g].connection,
            h = c.targetBlock();
        if (h) {
            var f = h.type;
            "math_arithmetic" == f || "math_modulo" == f || "library_math_abs" == f || "library_math_trig" == f || "library_math_logs" == f || "library_math_pow" == f || "library_math_exp" == f || "library_math_sqrt" == f || "library_math_round" == f || "library_string_strlen" == f || "library_stdlib_rand" == f || "library_stdlib_number_forRandScope1" ==
                f || "library_stdlib_number_forRandScope100" == f || "library_stdlib_sizeof_forMalloc" == f || "library_stdlib_arithmetic_forMalloc" == f || "library_stdlib_number_forMalloc" == f ? (d += "%d", e += ", " + b) : "library_string_strcat" == f || "library_string_strcpy" == f || "library_string_strcmp" == f ? (d += "%s", e += ", " + b) : "library_stdlib_convert" == f ? (-1 != b.indexOf("atoi(") ? d += "%d" : -1 != b.indexOf("atof(") && (d += "%f"), e += ", " + b) : "variables_array_get" == f ? (c = b.split("["), c = Blockly.cake.varTypeCheckInPrintScan(c[0]), "" == c ? d += b : (d += c, e += ", " +
                    b)) : "variables_pointer_get" == f ? (d += "%p", e += ", " + b) : "variables_pointer_&" == f ? h.inputList[0].connection.targetBlock() && (b = Blockly.cake.valueToCode(h, "VALUE", Blockly.cake.ORDER_NONE) || "", d += "%p", e += ", &" + b) : "variables_pointer_*" == f ? h.inputList[0].connection.targetBlock() && (b = Blockly.cake.valueToCode(h, "VALUE", Blockly.cake.ORDER_NONE) || "", 0 <= b.indexOf("*") && h.inputList[0].connection.targetBlock().inputList[0].connection.targetBlock() ? (b = Blockly.cake.valueToCode(h.inputList[0].connection.targetBlock(),
...
Blockly.cake.library_stdio_scanf = function(a) {
    for (var b = "", c = "", d = "", e = "", g = 0; g <= a.scanAddCount_; g++) {
        var b = Blockly.cake.valueToCode(a, "VAR" + g, Blockly.cake.ORDER_NONE) || "",
            c = a.inputList[g].connection,
            h = c.targetBlock();
        if (h) {
            var f = h.type;
            "variables_array_get" == f ? (h = b.split("["), c = Blockly.cake.arrTypeCheckInScan(h[0], c), "%s" == c ? (d += c, e += ", " + b) : (d += c, e += ", &" + b)) : "variables_pointer_get" == f ? (c = Blockly.cake.varTypeCheckInPrintScan(b), "%c" == c && (c = "%s"), d += c, e += ", " + b) : "variables_pointer_&" == f ? (c.isSuperior() ?
                c.targetBlock().setParent(null) : c.sourceBlock_.setParent(null), c.sourceBlock_.bumpNeighbours_()) : "variables_pointer_*" == f ? h.inputList[0].connection.targetBlock() && (b = Blockly.cake.valueToCode(h, "VALUE", Blockly.cake.ORDER_NONE) || "", c = Blockly.cake.varTypeCheckInPrintScan(b), "" == c ? d += b : (d += c, e += ", &*" + b)) : (c = Blockly.cake.varTypeCheckInPrintScan(b), d += c, e += ", &" + b)
        }
    }
    Blockly.cake.definitions_.include_cake_stdio = "#include <stdio.h>";
...
Blockly.cake.pointerTypeCheckInPrint = function(a, b) {
    ...
    if (1 == b) {
        for (var e = 0; e < d.length; e++)
            if (a == d[e][2]) {
                var g = d[e][0];
                "dbint" == g ? c = "%d" : "dbunsigned int" == g ? c = "%u" : "dbfloat" == g ? c = "%f" : "dbdouble" == g ? c = "%f" : "dbchar" == g && (c = "%c");
                break
            } return c
    }
    for (e = 0; e < d.length; e++)
        if (a == d[e][2]) return g = d[e][0], "*" == d[e][5] ? "int" == g ? c = "%d" : "unsigned int" == g ? c = "%u" : "float" == g ? c = "%f" : "double" == g ? c = "%f" : "char" == g && (c = "%c") : c = "dbchar" == g ? "%s" : "%p", c
};
Blockly.cake.arrTypeCheckInScan = function(a, b) {
    for (var c = "", d = b.targetBlock(), e = Blockly.Blocks.getWantedBlockArray("a"), g = 0; g < e.length; g++) {
        var h = Blockly.FieldDropdown.prototype.getTypefromVars(e[g][2], 0),
            f = e[g][5][0],
            k = d.getInputIdxLength();
        if (f == k) {
            "int" == h ? c = "%d" : "unsigned int" == h ? c = "%u" : "float" == h ? c = "%f" : "double" == h ? c = "%f" : "char" == h ? c = "%c" : "dbchar" == h && (c = "%s");
            break
        } else if (f > k && "char" == e[g][0]) {
            c = "%s";
            break
...
Blockly.cake.library_stdlib_rand = function(a) {
    var b;
    b = Blockly.cake.valueToCode(a, "VAR", Blockly.cake.ORDER_NONE) || "";
    Blockly.cake.definitions_.include_cake_stdlib = "#include <stdlib.h>";
...
Blockly.cake.structure_define = function(a) {
    for (var b = Blockly.cake.variableDB_.getName(a.getFieldValue("NAME"), null), c = [], d = [], e = [], g = [], h = [], f = 0; f < a.members_.length; f++) c[f] = Blockly.cake.variableDB_.getName(a.members_[f], Blockly.Variables.NAME_TYPE), d[f] = a.types_[f], e[f] = a.dist_[f], g[f] = a.spec_[f], "v" == e[f] ? h[f] = d[f] + " " + c[f] + ";\n" : "a" == e[f] ? h[f] = d[f] + " " + c[f] + "[" + g[f] + "];\n" : "p" == e[f] && (h[f] = d[f] + g[f] + " " + c[f] + ";\n");
...
Blockly.cake.variables_array_get = function(a) {
    var b = Blockly.cake.variableDB_.getName(a.getFieldValue("VAR"), Blockly.Variables.NAME_TYPE),
        b = Blockly.Blocks.checkUnselect(b),
        c = a.getFieldValue("LENGTH_1"),
        d = a.getFieldValue("LENGTH_2"),
        e = a.getFieldValue("LENGTH_3");
    0 == isNaN(c) && (c = "" == c ? -1 : 1 * c);
    0 == isNaN(d) && (d = "" == d ? -1 : 1 * d);
    0 == isNaN(e) && (e = "" == e ? -1 : 1 * e);
    var g = Blockly.Blocks.getWantedBlockArray("a"),
        h = Blockly.Blocks.getIndexArray(g, b),
        f, k, g = Blockly.Blocks.checkArrayIndex(c, h[0]);
    k = Blockly.Blocks.checkArrayIndex(d,
        h[1]);
    h = Blockly.Blocks.checkArrayIndex(e, h[2]);
    0 == g && -1 != c || 0 == k && -1 != d || 0 == h && -1 != e ? (window.alert("\uc778\ub371\uc2a4 \ucd08\uacfc"), a.initIdx(g, k, h)) : 1 == g && 0 == k && 0 == h ? f = b + "[" + c + "]" : 1 == g && 1 == k && 0 == h ? f = b + "[" + c + "][" + d + "]" : 1 == g && 1 == k && 1 == h ? f = b + "[" + c + "][" + d + "][" + e + "]" : 0 == g && 0 == k && 0 == h ? (a = this.getFieldValue("VAR"), a = Blockly.FieldVariableArray.getBlockIdxLength(a), f = 1 == a ? b + "[]" : 2 == a ? b + "[][]" : b + "[][][]") : a.initIdx(g, k, h);
    return [f, Blockly.cake.ORDER_ATOMIC]
};
Blockly.cake.variables_array_set = function(a) {
    var b = Blockly.cake.valueToCode(a, "VALUE", Blockly.cake.ORDER_ASSIGNMENT) || "0",
        c = Blockly.cake.variableDB_.getName(a.getFieldValue("VAR"), Blockly.Variables.NAME_TYPE),
        c = Blockly.Blocks.checkUnselect(c),
        d = a.getFieldValue("LENGTH_1"),
        e = a.getFieldValue("LENGTH_2"),
        g = a.getFieldValue("LENGTH_3"),
        d = "" == d ? -1 : 1 * d,
        e = "" == e ? -1 : 1 * e,
        g = "" == g ? -1 : 1 * g,
        h = Blockly.Blocks.getWantedBlockArray("a"),
        f = Blockly.Blocks.getIndexArray(h, c),
        k, l, h = Blockly.Blocks.checkArrayIndex(d, f[0]);
    l = Blockly.Blocks.checkArrayIndex(e, f[1]);
    f = Blockly.Blocks.checkArrayIndex(g, f[2]);
    0 == h && -1 != d || 0 == l && -1 != e || 0 == f && -1 != g ? (window.alert("\uc778\ub371\uc2a4 \ucd08\uacfc"), a.initIdx(h, l, f)) : 1 == h && 0 == l ? k = c + "[" + d + "] = " + b + ";\n" : 1 == h && 1 == l && 0 == f ? k = c + "[" + d + "][" + e + "] = " + b + ";\n" : 1 == h && 1 == l && 1 == f ? k = c + "[" + d + "][" + e + "][" + g + "] = " + b + ";\n" : a.initIdx(h, l, f);
    return k
};
Blockly.cake.variables_array_declare = function(a) {
    var b = Blockly.cake.valueToCode(a, "VALUE", Blockly.cake.ORDER_ASSIGNMENT) || "0",
        ...
        g = a.getFieldValue("LENGTH_2");
    a = a.getFieldValue("LENGTH_3");
    var h;
    0 != e && 0 == g && 0 == a ? h = d + " " + c + "[" + e + "] = {" + b + "};\n" : 0 != e && 0 != g && 0 == a ? h = d + " " + c + "[" + e + "][" + g + "]  = {" + b + "};\n" : 0 != e && 0 != g && 0 != a && (h = d + " " + c + "[" + e + "][" + g + "][" + a + "] = {" +
...
Blockly.cake['io_digitalread'] = function(block) {
 ...
 if(Blockly.cake.outputs.indexOf(dropdown_name)!=-1)
    console.log("Chutiya ho gaya kya"+ dropdown_name);

 if(Blockly.cake.inputs.indexOf(dropdown_name)==-1)
    Blockly.cake.inputs.push(dropdown_name);
  // TODO: Assemble JavaScript into code variable.
...
Blockly.cake['io_digitalwrite'] = function(block) {
  var dropdown_pins = block.getFieldValue('pins');
  console.log(this.setWarning);

  var value_state = Blockly.cake.valueToCode(block, 'state', Blockly.cake.ORDER_ATOMIC);
  var index=Blockly.cake.outputs.indexOf(this.previousValue); 
 if(index!=-1)
    Blockly.cake.outputs.splice(index,1)

 if(Blockly.cake.inputs.indexOf(dropdown_pins)!=-1)
        this.setWarningText("Pin "+dropdown_pins+" is alraedy used as inputs");
else
    this.setWarningText(null);

 if(Blockly.cake.outputs.indexOf(dropdown_pins)==-1)
...
Blockly.cake['io_highlow'] = function(block) {
